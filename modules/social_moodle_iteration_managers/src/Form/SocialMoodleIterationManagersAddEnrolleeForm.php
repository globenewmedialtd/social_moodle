...
use Drupal\node\NodeInterface;
use Drupal\social_moodle_iteration_managers\Element\SocialMoodleIterationManagersEnrollmentAutocomplete;

/**
 * Class SocialIterationManagersAddEnrolleeForm.
 *
 * @package Drupal\social_moodle_iteration_managers\Form
 */
class SocialMoodleIterationManagersAddEnrolleeForm extends FormBase {
  ...
  public function getFormId() {
    return 'social_moodle_iteration_managers_enrollment_add';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $iteration = $form_state->getValue('node_id');
    $count = 0;

    if (!empty($iteration) && !empty($enroll_uid)) {
      // Create a new enrollment for the Iteration.
      foreach ($enroll_uid as $uid => $target_id) {
        $enrollment = IterationEnrollment::create([
      ...
      // Add nice messages.
      if (!empty($count)) {
        $message = $this->formatPlural($count, '@count new member is enrolled to this iteration.', '@count new members are enrolled to this iteration.');

        if (social_moodle_enrollment_iteration_manager_or_organizer(NULL, TRUE)) {
          $message = $this->formatPlural($count, '@count new member is enrolled to your iteration.', '@count new members are enrolled to your iteration.');
        }
        \Drupal::messenger()->addMessage($message, 'status');
      }

      // Redirect to management overview.
      $url = Url::fromRoute('view.iteration_manage_enrollments.page_manage_enrollments', [
        'node' => $iteration,
      ]);
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $storage = $this->entityTypeManager->getStorage('iteration_enrollment');
    $enrollments = $storage->loadByProperties(['field_iteration' => $nid]);
    ...
    $add_directly_config = $this->configFactory->get('message.template.member_added_by_it_organiser')->getRawData();
    ...
    $form['actions']['cancel'] = [
      '#type' => 'link',
      '#title' => t('Cancel'),
      '#url' => Url::fromRoute('view.iteration_manage_enrollments.page_manage_enrollments', ['node' => $nid]),
    ];
  ...
  public function uniqueMembers($element, &$form_state, $complete_form) {
    ...
    SocialMoodleIterationManagersEnrollmentAutocomplete::validateEntityAutocomplete($element, $form_state, $complete_form, TRUE);
  }
