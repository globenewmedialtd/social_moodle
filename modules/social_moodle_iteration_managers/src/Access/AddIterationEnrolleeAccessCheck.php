...
namespace Drupal\social_moodle_iteration_managers\Access;
 ...
 */
class AddIterationEnrolleeAccessCheck implements AccessInterface {
  ...
  public function access(Route $route, RouteMatchInterface $route_match, AccountInterface $account) {
    $permission = $route->getRequirement('_iteration_enrollee_permission');

    // Don't interfere if no permission was specified.
    // Or it's not the permission we are looking for.
    if ($permission === NULL || $permission !== 'manage everything iteration enrollments') {
    ...
    if ($account->hasPermission('manage everything iteration enrollments')) {
      return AccessResult::allowed();
    }
    $type = $node->getType();
    // Don't interfere if it's not an iteration.
    if ($type !== 'iteration') {
      return AccessResult::neutral();
    }
    ...
    // Lets return the correct access for our Iteration Manager.
    $managers_access = SocialMoodleIterationManagersAccessHelper::getEntityAccessResult($node, 'update', $account);
