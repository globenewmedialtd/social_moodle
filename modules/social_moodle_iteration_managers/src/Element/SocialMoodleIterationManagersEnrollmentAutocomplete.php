...
namespace Drupal\social_moodle_iteration_managers\Element;
 ...
 *
 * @FormElement("social_moodle_iteration_managers_enrollment_entity_autocomplete")
 */
class SocialMoodleIterationManagersEnrollmentAutocomplete extends EntityAutocomplete {
  ...
  public static function validateEntityAutocomplete(array &$element, FormStateInterface $form_state, array &$complete_form, $select2 = FALSE) {
    ...
    foreach ($input_values as $input) {
      ...
      if ($match !== NULL) {
        ...
        $enrollments = $storage->loadByProperties(['field_iteration' => $nid, 'field_account' => $match]);

        // If the social_moodle_iteration_invite module is enabled, we want to check if
    ...
    if (!empty($duplicated_values)) {
      ...
      if (\Drupal::moduleHandler()->moduleExists('social_moodle_iteration_invite') && $invited_or_joined === TRUE) {
        $message = \Drupal::translation()->formatPlural(count($duplicated_values),
