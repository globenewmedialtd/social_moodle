...
namespace Drupal\social_moodle_iteration_invite\Plugin\views\field;
 ...
 *
 * @ViewsField("social_moodle_iteration_invite_recipient")
 */
class SocialMoodleIterationInviteRecipientField extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    // and get the iteration_enrollment__field_email value.
    $build = [];

    // Check the database for the email and account.
    $recipient = $this->checkEmailAndAccount($values->id, $values->users_field_data_iteration_enrollment__field_account_uid);
  ...
  private function checkEmailAndAccount($enrollment_id, $account_id) {
    ...
    $emailQuery = $this->database->select('iteration_enrollment__field_iteration', 'eefev');
    $emailQuery->join('iteration_enrollment__field_email', 'eefem', 'eefem.entity_id = ' . $enrollment_id);
